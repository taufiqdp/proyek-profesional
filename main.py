...
def load_data(uploaded_file):
    if uploaded_file is not None:
        try:
            ...
            required_columns = ['date', 'sentiment', 'tweet_text']
            if not all(col in df.columns for col in required_columns):
                st.error("CSV file must contain 'date', 'sentiment', and 'tweet_text' columns.")
                return None
        ...
        # Placeholder data if no file is uploaded
        df = pd.DataFrame({
            'date': pd.date_range(start='2023-01-01', periods=100),
            'sentiment': ['positive', 'negative', 'neutral'] * 33 + ['positive'],
            'tweet_text': ['Sample tweet ' + str(i) for i in range(100)],
        })
        return df
...
if df is not None:
    ...
    with col2:
        ...
        sentiment_over_time = filtered_df.groupby([filtered_df['date'].dt.date, 'sentiment']).size().unstack(fill_value=0)
    ...
    text = " ".join(tweet for tweet in filtered_df['tweet_text'])
    ...
    for _, tweet in sample_tweets.iterrows():
        st.text_area(f"Tweet ({tweet['sentiment']})", tweet['tweet_text'], height=100)
